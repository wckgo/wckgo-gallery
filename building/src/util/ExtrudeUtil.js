...
export function getExtrudeGeometry(polygon, height, layer, center) {
    return getExtrudeGeometryParams(polygon, height, layer, center);
...
export function getExtrudeGeometryParams(polygon, height, layer, center) {
    ...
    const shapes = datas[0][0];
    //Possible later use of geojson
    if (!shapes) return null;
    const depth = layer.distanceToVector3(height, height).x;
    const points = shapes.map(shape => new THREE.Vector2(shape[0], shape[1]));
    const s = new THREE.Shape(points);
    return new THREE.ExtrudeBufferGeometry(s, {
        steps: height - 30,
        depth: depth,
        bevelEnabled: false 
    })
